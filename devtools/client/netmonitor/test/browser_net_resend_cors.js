...
add_task(function* () {
  ...
  for (let [i, method] of METHODS.entries()) {
    let { data } = RequestsMenu.getItemAtIndex(i);
    is(data.method, method, `The ${method} request has the right method`);
    is(data.url, requestUrl, `The ${method} request has the right URL`);
  }
  ...
  for (let [i, method] of METHODS.entries()) {
    let index = i + 2;
    let item = RequestsMenu.getItemAtIndex(index).data;
