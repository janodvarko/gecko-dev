...
add_task(function* () {
  let { filterFreetext } = require("devtools/client/netmonitor/actions/index");

  let { monitor } = yield initNetMonitor(FILTERING_URL);

  function setFreetextFilter(value) {
    RequestsMenu.store.dispatch(filterFreetext(value));
  }
  ...
  function testContents(visibility) {
    ...
    is(items.length, visibility.length,
      "There should be a specific amount of items in the requests menu.");
    is(visibleItems.length, visibility.filter(e => e).length,
